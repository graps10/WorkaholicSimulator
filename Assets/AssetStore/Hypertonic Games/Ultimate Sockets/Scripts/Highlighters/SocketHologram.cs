...
namespace Hypertonic.Modules.UltimateSockets.Highlighters
{
    public class SocketHologram : MonoBehaviour, ISocketHighlighter
    {
        ...
        public void SetHologramRotation(PlaceableItem item)
        {
            if (_activeHologramGameObject==null) return;
            if (item.ItemTag.Contains("Rotating")) return;
            /*if (_socket is not EquipmentSocket socket) return;


            if (socket.defaultRotation.eulerAngles.x != 0)
            {
                _activeHologramGameObject.transform.localRotation = socket.defaultRotation;
                return;
            }

            float zRotation = item.RootTransform.localEulerAngles.z;
            Quaternion zRotationQuat = Quaternion.Euler(0f, 0f, zRotation);
            _activeHologramGameObject.transform.localRotation = socket.defaultRotation * zRotationQuat;*/
        }
